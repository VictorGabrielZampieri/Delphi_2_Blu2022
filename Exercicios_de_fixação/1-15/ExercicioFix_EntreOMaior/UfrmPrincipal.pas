...
begin
  ...
  if (xPrimeiro > xSegundo) then
    ShowMessage(xPrimeiro.ToString + ' É maior que ' + xSegundo.ToString);

  if (xSegundo > xPrimeiro) then
    ShowMessage(xSegundo.ToString + ' É maior que ' + xPrimeiro.ToString);

  if (xPrimeiro = xSegundo) or (xSegundo = xPrimeiro) then
    ShowMessage('Os Dois Numero São Iguais');
end;
