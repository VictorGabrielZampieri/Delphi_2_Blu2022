...
begin
  ...
  if not TryStrToFloat(edtNota1.Text, xPrimeiraNota) then
    raise Exception.Create('Por Favor Insira um Valor Válido na 1º Nota');

  if not TryStrToFloat(edtNota2.Text, xSegundaNota) then
    raise Exception.Create('Por Favor Insira um Valor Válido na 2º Nota');

  if not TryStrToFloat(edtNota3.Text, xTerceiraNota) then
    raise Exception.Create('Por Favor Insira um Valor Válido na 3º Nota');
...
begin
  ...
  lblMedia.Caption := 'Média Final Do Aluno : ' + FormatFloat('0.00',xMediaAluno);
  lblSituacaoAluno.Caption := 'Situação Do Aluno : ' + xSituacaoAluno;
end;
