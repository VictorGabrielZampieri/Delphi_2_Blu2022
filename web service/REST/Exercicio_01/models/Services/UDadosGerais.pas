...
type
  TDadosGerais = class
    ...
    public
      ...
      function AdicionarProduto(aJSON: TJSONObject): Boolean;
      function RemoverProduto(const aIndentificador: Integer): Boolean;
...
function TDadosGerais.AdicionarProduto(aJSON: TJSONObject): Boolean;
begin
  try
...
begin
  ...
  try
    try
      ...
      on E: Exception do
        raise Exception.Create('Nâo foi possível Carregar os Dados '+ E.Message);
    end;
  finally
...
function TDadosGerais.RemoverProduto(const aIndentificador: Integer): Boolean;
var
  I: Integer;
  xJSON: TJSONObject;
begin
  if not Assigned(FJSON) then
    raise Exception.Create('Dado não encontrado');
  ...
  try
    ...
    begin
      ...
      if (xJSON.GetValue('id').Value.ToInteger = aIndentificador) then
      begin
        FJSON.Remove(I).Free;
        Self.SalvarDados;
